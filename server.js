...
class DatabaseManager {
  ...
  async addUser(stakeUsername, subscriptionType, subscriptionStart, subscriptionEnd) {
    await this.connect();
  ...
  async updateUserSubscription(stakeUsername, subscriptionType, subscriptionEnd) {
    await this.connect();
  ...
  async verifyPromoCode(promoCode, subscriptionType) {
    ...
    if (!promo.applicableDurations.includes(subscriptionType)) {
      return {
        isValid: false,
        message: "Ce code promo n'est pas applicable pour la durée d'abonnement sélectionnée.",
      };
    }

    // Décrémenter la limite d'utilisation
    await this.promos.updateOne(
      { code: promoCode },
      { $inc: { usageLimit: -1 } }
    );
